...
maxv = np.sqrt( (2.*thrust*1000) / (rho * c_drag * 2*aside*wlgth) ) #maximum speed where drag balances thrust
takeoff = np.sqrt( (2.*mass*1000*10) / (rho * c_lift * 2*atop*wlgth) ) #cruise speed/takeoff speed (lift = weight)
...
class Panel(wx.Panel):
    ...
    def Integrate(self):
        ...
        if (not self.crash):
            ...
            thrust2=thrust
            lift = c_lift*atop*wlgth*2 * 0.5*rho*self.vx*self.vx
            drag = c_drag*aside*wlgth*2 * 0.5*rho*self.vx*self.vx
...
if __name__ == '__main__':
    app = wx.App()
    frame = wx.Frame(None, -1, 'Airport',size=(1200,675))
    panel = Panel(frame, 'background.png')
    frame.Show()
    app.MainLoop()
