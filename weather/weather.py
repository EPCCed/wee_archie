...
def GetVTKData(root): #root=netcdf handle
...
def GetFilename(num): return "data/data%5.5d.nc"%num #get filename
...
def process(frameno,pipe): # frameno=shared variable containing current frame number, pipe = communication object
    print "Process initiated"
    while True: #infinite loop
        frame=frameno.value #get frame number
        
        if (frame >= 0): #we have to read a file
            fname=GetFilename(frame)
            try: #if there is a file of that name:
                
                root=Dataset(fname,"r")
                
                data=GetVTKData(root)
...
class MyWindow(wx.Frame):
    ...
    def __init__(self,parent,title):
        ...
        self.p=mp.Process(target=process,args=(self.frameno,pipeprocess))
...
#need to set up a wx app (false means stderr/stdout is not sent to the window but to the terminal)
app=wx.App(False)
window=MyWindow(None,"Galaxy simulator")
#start eventloop
app.MainLoop()
